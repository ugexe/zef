...
Zef - Perl6 Module Management (GLR compatible)
...
Zef is a package manager for Perl6. It works with MoarVM and JVM on Windows, Linux, OSX, and other BSD derivatives.

=head3 Features

=over 2

=item * Optonal parallelization of precompilation and testing phases for quicker installs and CI results.

=item * Complete HTTP grammar based on RFC7230-7235 (and others).

=item * Zef::Net::HTTP::Client - a HTTP client based on included HTTP grammars.

=item * CompUnitRepo based installs - uses rakudo's built in installer, letting the code focus on package management.

=item * A `hooks` implementation as mentioned in L<S22|http://design.perl6.org/S22.html#hooks>, allowing module authors to even implement the entire build/test/install process without needing a package manager at all.

=item * No external dependencies required. Can bootstrap itself from source using the same zef application used to install all other modules.
...
B<Note>: I<Legacy hooks> (Build.pm)

For modules written before S22 you may encounter I<Build.pm> files. These will be detected as legacy hooks and fired during `before-build`.

A common problem is modules that use `Build.pm` usually do not declare the dependencies used in their `Build.pm`. The S22 hook implementation Zef uses is not tied to any specific package management implementation and thus has no inherit dependencies. This also means module authors can support the new hook implementation through `Build.pm` itself, allowing package managers that do not use hooks to still `just work` with the S22 spec.
