...
Note that not every `ContentStorage` will provide such a list. An example is we
don't have a way to get a list of every distribution stored on metacpan (however
we *do* have access to the latest $count releases, so maybe it would be acceptable
for such storages to list just those)
