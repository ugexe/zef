...
=head1 USAGE
    ...
    # Upgrade specific distributions (BETA)
...
Note: Some `ContentStorage` backends, like the default Ecosystems, have an `auto-update` option
in `resources/config.json` that can be enabled

=head4 B<upgrade> [*@identities] I<BETA>

Upgrade specified identities. If no identities are provided, it attempts to upgrade all installed distributions.
