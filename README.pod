...
Please use --force or --notests on the `install Zef` command to intall on MoarVM for now, ignoring any test failures. Installing other modules still works without these workarounds. 
