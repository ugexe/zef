...
=head1 USAGE
    ...
    # (the `.pm` extension is also recognized)
    zef build .
...
NOTE: You may use installed modules inside the build hook, but those modules
must be listed in the `META6.json` file in either the "build-depends" or "depends"
section, as appropiate. You also may use the module being built by adding
something similar to the following in the build hook file:

    use lib <./lib>; # ensures the local version is being used
    use MyNewModule :ALL;
