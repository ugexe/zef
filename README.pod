...
Maybe. Paths are considered valid if it can be turned into a valid B<IO> object. URIs are validated with a URI grammar.
I don't claim to be a security expert, so if you notice something that could be improved i'll be happy to listen.
