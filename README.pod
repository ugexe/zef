...
=head4 topological-sort(*@meta-files, :$target)

Build dependency order, in levels, from a list of dependency meta files. If :$target is passed then the tree will built with :$target as the base node. Otherwise the order will determine a base node(s) in the form of @level[*-1]
