...
[C<@from>] allows you to show results from specific storages only:
    ...
    zef list cpan               # Only show available modules from the content storage
                                # with a name field of `cpan` in the configuration
                                # (plugin must not be set to `"enabled" : 0`)
...
Fetches candidates for given identities, including any dependencies (if requested)
...
You can enable it by setting I<"enabled" : 1> in the config under the ContentStorage::CPAN plugin, or temporarily by passing a flag:

    # flag
    -CContentStorage="cpan.enabled=1"

    # examples
    $ zef -v -CContentStorage="cpan.enabled=1" search CSV::Parser
    $ zef -v -CContentStorage="cpan.enabled=1" install CSV::Parser
...
Pass a path to the I<-to> / I<--install-to> option and prefix the path with `inst#` (unless you know what you're doing)
    ...
    $ perl6 -Ilib bin/zef -to="inst#/home/perl6/custom" install Text::Table::Simple
    Searching for Text::Table::Simple
    Start test phase for: /home/perl6/.zef/store/Perl6-Text--Table--Simple.git/09abf9d5d59e9dfdc1be24319db0dcd76b764ede
    Testing passed for /home/perl6/.zef/store/Perl6-Text--Table--Simple.git/09abf9d5d59e9dfdc1be24319db0dcd76b764ede
    Installing Text::Table::Simple:ver('0.0.3'):auth('github:ugexe') to /home/perl6/custom
