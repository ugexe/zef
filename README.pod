...
=head1 INSTALLATION
    ...
    cd zef
    perl6 -Ilib bin/zef -v --no-build install .
...
Be sure your `PATH` includes the path to perl6 scripts:
    ...
    # not using rakudobrew? Add the command output below to your `PATH`
    $ perl6 -e "say %*CUSTOM_LIB<site>.IO.child('bin').Str"
    /home/perl6/install/share/perl6/site/bin

=head2 Notice

Rakudo does not currently load any precompiled module code. It is suggested to use the C<--no-build> flag with C<install> until such a time that it does.
